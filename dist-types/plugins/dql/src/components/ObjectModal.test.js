...
const prepareComponent = async ({ data = '{}', property = 'my prop', } = {}) => {
    return await renderInTestApp(React.createElement(ObjectModal, { data: data, property: property }));
};
const getOpenModalButton = () => screen.getByRole('button', { name: 'Complex record' });
...
describe('ObjectModal', () => {
    it('should open the modal with JSON data', async () => {
        ...
        expect(screen.getByRole('dialog')).toHaveTextContent('"myProperty":"My value"');
    });

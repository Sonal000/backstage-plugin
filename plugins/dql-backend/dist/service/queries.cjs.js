...
var labelSelectorParser = require('../utils/labelSelectorParser.cjs.js');

const isValidDynatraceQueryKey = (key) => key in dynatraceQueries;
const dynatraceQueries = {
  ["kubernetes-deployments" /* KUBERNETES_DEPLOYMENTS */]: (entity, apiConfig) => {
    const labelSelector = entity.metadata.annotations?.["backstage.io/kubernetes-label-selector"];
    const kubernetesId = entity.metadata.annotations?.["backstage.io/kubernetes-id"];
    const namespace = entity.metadata.annotations?.["backstage.io/kubernetes-namespace"];
    const filterLabel = labelSelector ? labelSelectorParser.generateKubernetesSelectorFilter(labelSelector) : "";
    const filterKubernetesId = filterLabel || !kubernetesId ? "" : `| filter workload.labels[\`backstage.io/kubernetes-id\`] == "${kubernetesId}"`;
    const filterNamespace = namespace ? `| filter Namespace == "${namespace}"` : "";
  ...
  ["srg-validations" /* SRG_VALIDATIONS */]: (entity, apiConfig) => {
    const catalogTags = entity.metadata.annotations?.["dynatrace.com/guardian-tags"];
    let filterString = "";
    if (catalogTags) {
      const tags = catalogTags.split(",").map((tag) => {
        const [key, value] = tag.split("=");
        return { key, value: value !== void 0 ? value || "" : void 0 };
      });
      if (tags.length > 0) {
        filterString += "| filter ";
        filterString += tags.map(
          ({ key, value }) => value !== void 0 ? `in (tags[\`${key}\`], "${value}")` : `isNotNull(tags[${key}])`
        ).join(" AND ");
      }
    }
    return `
...
exports.dynatraceQueries = dynatraceQueries;
exports.isValidDynatraceQueryKey = isValidDynatraceQueryKey;
//# sourceMappingURL=queries.cjs.js.map

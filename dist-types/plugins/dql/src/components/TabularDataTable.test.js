...
const prepareComponent = ({ title = 'some title', data = [], pageSize, }) => {
    return render(React.createElement(TabularDataTable, { title: title, data: data, pageSize: pageSize }));
};
describe('TabularDataTable', () => {
    ...
    it('should render anything else as JSON', () => {
        ...
        const rendered = prepareComponent({ data });
        expect(rendered.getByText(JSON.stringify(data[0].Header))).toBeInTheDocument();
    });
    ...
    it('should filter by the entered text', async () => {
        ...
        waitFor(() => {
            expect(rendered.getByText('other data').closest('td')).toBeInTheDocument();

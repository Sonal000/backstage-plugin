 ...
 */
export const DqlQuery = (props) => {
    try {
        const { title, queryId } = dqlQueryPropsSchema.parse(props);
        const [queryNamespace, queryName] = queryId.split('.');
        return (React.createElement(InternalDqlQuery, { title: title, queryNamespace: queryNamespace, queryName: queryName, EmptyState: props.emptyState, pageSize: props.pageSize }));
    }
    catch (e) {
        if (e instanceof ZodError) {
            const title = `${e.issues[0].path.join('.')}: ${e.issues[0].message}`;
            return React.createElement(ErrorPanel, { error: e, title: title });
        }
        return React.createElement(ErrorPanel, { error: new Error(`Unknown error: ${e}`) });
    }
};

...
describe('queries', () => {
    ...
    describe(DynatraceQueryKeys.KUBERNETES_DEPLOYMENTS, () => {
        it('should fail if neither the label selector nor the kubernetes id annotation is provided', () => {
            // act, assert
            expect(() => dynatraceQueries[DynatraceQueryKeys.KUBERNETES_DEPLOYMENTS](getEntity(), defaultApiConfig)).toThrow();
        });
        it('should return the query without the kubernetesId filter if a label selector is provided', () => {
            // act
            const query = dynatraceQueries[DynatraceQueryKeys.KUBERNETES_DEPLOYMENTS](getEntity({
                'backstage.io/kubernetes-id': 'kubernetesId',
                'backstage.io/kubernetes-label-selector': 'label=value',
            }), defaultApiConfig);
            // assert
            expect(query).not.toContain('| filter workload.labels[`backstage.io/kubernetes-id`] == "kubernetesId"');
            expect(query).toContain('| filter workload.labels[`label`] == "value"');
        });
        it('should return the query with the kubernetesId filter ', () => {
            // act
            const query = dynatraceQueries[DynatraceQueryKeys.KUBERNETES_DEPLOYMENTS](getEntity({ 'backstage.io/kubernetes-id': 'kubernetesId' }), defaultApiConfig);
            // assert
            expect(query).toContain('| filter workload.labels[`backstage.io/kubernetes-id`] == "kubernetesId"');
        });
    ...
    describe(DynatraceQueryKeys.SRG_VALIDATIONS, () => {
        it('should return the srg-query', () => {
            // act
            const query = dynatraceQueries[DynatraceQueryKeys.SRG_VALIDATIONS](getEntity(), defaultApiConfig);
            // assert
            expect(query).toContain('fetch bizevents');
            expect(query).toContain('| filter event.provider == "dynatrace.site.reliability.guardian"');
        });
        it('should return the srg-query with filtering for tags defined in guardian-tags annotation', () => {
            // act
            const query = dynatraceQueries[DynatraceQueryKeys.SRG_VALIDATIONS](getEntity({
